...
- 验收：
  - [x] setTheme 触发订阅通知；刷新后从 localStorage 恢复
  - [x] 无存储记录时 mode = 系统偏好
- 预估：15min | 依赖：P1 | 状态：done
...
- 验收：
  - [x] 切换主题后 `:root` 变量全量更新（DevTools 验证）
  - [x] 112 处 CSS 变量消费者无改动、显示正确
- 预估：10min | 依赖：2.1 | 状态：done
...
- 验收：
  - [x] 切换 light/dark 时 antd 组件（Menu/Button/Card/Bubble）随动
  - [ ] `npm run test` 全绿（含 1.4 重构后的 themeConfig 测试）
- 预估：15min | 依赖：2.2 | 状态：done
